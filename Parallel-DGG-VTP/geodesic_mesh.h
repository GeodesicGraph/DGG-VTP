...
public:
	...
	bool IsDegenerate() { return IsDegenerateMesh; }
...
inline void Mesh::build_adjacencies()
{
	...
	for(unsigned i = 0; i < m_faces.size(); ++i)
	{
		...
		for(unsigned j = 0; j < 3; ++j)
		{
			...
			half_edges[k].vertex_0 = min(vertex_id_1, vertex_id_2);
			half_edges[k].vertex_1 = max(vertex_id_1, vertex_id_2);
...
inline void Mesh::calculate_statics_data()
{
	...
	for (unsigned i = 0; i < m_edges.size(); ++i)
	{
		...
		max_edge_len = max(max_edge_len, m_edges[i].length());
		min_edge_len = min(min_edge_len, m_edges[i].length());
	}
...
inline bool Mesh::verify()		//verifies connectivity of the mesh and prints some debug info
{
	...
	for(unsigned i = 0; i < m_edges.size(); ++i)
	{
		...
		longest_edge = max(longest_edge, e.length());
		shortest_edge = min(shortest_edge, e.length());
	}
