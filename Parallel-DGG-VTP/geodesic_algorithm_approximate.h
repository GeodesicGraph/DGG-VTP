...
#ifndef GEODESIC_ALGORITHM_APPROXIMATE_H
#define GEODESIC_ALGORITHM_APPROXIMATE_H
...
namespace geodesic{
	...
	class GeodesicAlgorithmApproximate : public GeodesicAlgorithmBase
	{
	public:
		GeodesicAlgorithmApproximate(geodesic::Mesh* mesh, double lambda) : 
			GeodesicAlgorithmBase(mesh),
			lambda(lambda)
		{};
		~GeodesicAlgorithmApproximate(){};
		...
		void clear(){
			...
			depth_create_pseudo = 0;
			mx_depth = 0;
			ratio_each_list.clear();
			avg_ratio_each_list = 0;
			var_ratio_each_list = 0;
	...
	private:
		...
		void propagate_local_exact_pseudo_source_at_wavefront();
		...
		//for statistics
		unsigned depth_create_pseudo;
		unsigned mx_depth;
		unsigned num_propagated_vertices;
		unsigned num_propagated_local_exact_vertex;
		unsigned num_propagated_pseudo_source_windows;
		unsigned num_rings;
		std::vector<double> ratio_each_list; // num_of_windows / geodesic_distance(down-vertex) for each window_list
		double avg_ratio_each_list;
		double var_ratio_each_list;
	};

	//----------------------simple functions-------------------
	inline void GeodesicAlgorithmApproximate::initialize_propagation_data()
	{
		clear();
	...
	inline void GeodesicAlgorithmApproximate::erase_from_queue(vertex_pointer v)
	{
		/*
		if (m_vertex_queue.count(v) > 1)
		{
			printf("%d\n", m_vertex_queue.count(v));
			printf("something wrong!\n");
			for (std::set<vertex_pointer, Vertex>::iterator it = m_vertex_queue.find(v); it != m_vertex_queue.end(); it = m_vertex_queue.find(v))
			{
	...
	inline void GeodesicAlgorithmApproximate::create_pseudo_source_windows(vertex_pointer &pseudo_source, bool inside_traversed_area)
	{
...
//			candidate->mother_list_id() = -1;
			...
			if ((inside_traversed_area) && ((edge_it->v0()->state() == Vertex::INSIDE) || (edge_it->v1()->state() == Vertex::INSIDE)))
	...
	//----------------- propagate a windows list (Rule 1) ---------------------
	inline void GeodesicAlgorithmApproximate::find_separating_point(list_pointer &list)
	{
	...
	inline void GeodesicAlgorithmApproximate::propagate_windows_to_two_edges(list_pointer &list)
	{
...
//					right_w->mother_list_id() = list->edge()->id();//
	...
	//----------------- pairwise windows checking (Rule 2) ----------------------
	inline void GeodesicAlgorithmApproximate::check_with_vertices(list_pointer &list)
	{
		if (list->empty()) return;

		const double LOCAL_EPSILON = 1e-20 * list->edge()->length();
		...
		while ((!list->empty()) && (iter != NULL))
		{
			...
			else
			{
				...
				if (v1->geodesic_distance() + w->stop() < d1 + eps_ICH)
					w_survive = false;

				d1 = w->d() + sqrt((w->start() - w->pseudo_x()) * (w->start() - w->pseudo_x()) + w->pseudo_y() * w->pseudo_y());
				if (v2->geodesic_distance() + e->length() - w->start() < d1 + eps_ICH)
					w_survive = false;
			}
	...
	inline windows_state GeodesicAlgorithmApproximate::check_between_two_windows(interval_pointer &w1, interval_pointer &w2)
	{
	...
	inline void GeodesicAlgorithmApproximate::pairwise_windows_checking(list_pointer &list)
	{
		if (list->empty()) return;
	...
	//----------------- approximate operation (Du Jie)----------------------------
	inline void GeodesicAlgorithmApproximate::propagate_local_exact_pseudo_source_at_wavefront()
	{
		while (!m_vertex_queue.empty())
		{
			...
			for (unsigned i = 0; i < vert->adjacent_edges().size(); i++)
			{
				...
				//delete windows on lists
				if (!interval_list_0(edge_it)->empty())
				{
					list_pointer list = interval_list_0(edge_it);
					interval_pointer iter = list->begin();
					interval_pointer iter_t;
					while (iter)
					{
				...
				{
					list_pointer list = interval_list_1(edge_it);
					interval_pointer iter = list->begin();
					interval_pointer iter_t;
					while (iter)
					{
		...
		//propagate local exact pseudo source
		while (!m_local_exact_vertex_queue.empty())
		{
			vertex_pointer vert = m_local_exact_vertex_queue.front();
			m_local_exact_vertex_queue.pop();
			m_vertex_queue.insert(vert);
		}
	}

	//------------------------- main operation ----------------------------
	inline void GeodesicAlgorithmApproximate::propagate_one_windows_list(list_pointer &list)
	{
		if (list->empty()) return;
	...
	//----------------------main entry---------------------
	inline void GeodesicAlgorithmApproximate::propagate(unsigned source)
	{
		//initialization
		m_source = source;
		...
		while (!m_vertex_queue.empty())
		{
			...
			if (vert->geodesic_distance() > radius)
			{
				...
				radius += lambda * this->m_mesh->avg_edge(); 
				propagate_local_exact_pseudo_source_at_wavefront();
				
				if (m_vertex_queue.empty())
					break;
	
				vert = *m_vertex_queue.begin();
			}
	...
	//---------------------- print statistics --------------------------
	inline void GeodesicAlgorithmApproximate::print_statistics()
	{
		...
		printf("--------------------Appr-VTP statistics-------------------\n");

		num_propagated_vertices += num_propagated_local_exact_vertex;

		//output data to excel file
		FILE *file = fopen("Appr_VTP.csv", "a");
		fprintf(file, "%.6lf, %d, %d, %d, %d, %.6lf\n", m_time_consumed, num_rings, num_propagated_vertices, m_windows_propagation, m_windows_peak, used_memory);
		fclose(file);
