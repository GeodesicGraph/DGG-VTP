...
public:
	...
	//for fwp_group 'bucket' structure
	unsigned Kmin, Kmax;
	...
	double d)
{
	...
	/*
	if (omp_get_num_threads() > 1) //for parallel statistics
	{
		int thread_id = omp_get_thread_num();
		num_propagated_windows_on_thread[thread_id]++;
	}
	else //for sequential statistics
	{
