//orginal source code of VTP: "Fast and Exact Discrete Geodesic Computation Based on Triangle-Oriented Wavefront Propagation" 
//authored by Yipeng Qin, Xiaoguang Han, Hongchuan Yu, Yizhou Yu and Jianjun Zhang

#ifndef GEODESIC_ALGORITHM_EXACT_H
#define GEODESIC_ALGORITHM_EXACT_H
...
namespace geodesic {
	...
	inline void GeodesicAlgorithmExact::print_statistics()
	{
		...
		std::cout << "uses about " << used_memory << "MB of memory" << std::endl;
		std::cout << "total interval propagation number " << m_windows_propagation << std::endl;
		std::cout << "maximum interval queue size is " << m_queue_max_size << std::endl << std::endl;

		//output data to .csv(excel file)
		FILE* file = fopen("VTP.csv", "a");
		fprintf(file, "%.6lf, %d, %lld, %d, %.6lf\n", m_time_consumed, num_propagated_vertices, m_windows_propagation, m_windows_peak, used_memory);
		fclose(file);
